...
/* Implementation of KCapFL data structure containing labeled points. */
public class KCapFL<LPoint extends LabeledPoint2D> {
	
	/* KCapFL contains an integer representing the number of points that can
	 * be within a radius of a point, an extended kd-tree to store points,
	 * and a leftist heap to store key-value pairs representing the
	 * distances of pairs to a point. */
	private int capacity;
	private XkdTree<LPoint> kdTree;
	private LeftistHeap<Double, ArrayList<LPoint>> heap;

	/* Constructor for KCapFL sets the capacity to the provided value,
	 * creates an extended kd-tree with the given bucket size and bounding
	 * box, and a new empty leftist heap. */
	...
	/* Clears the contents of KCapFL by invoking clear() on its extended
	 * kd-tree and leftist heap. */
	public void clear() {
		this.kdTree.clear();
		this.heap.clear();
	}

	/* Stores a given list of points into KCapFL by inserting them into the
	 * extended kd-tree and calculating the k nearest neighbors for each point
	 * to store the kth nearest neighbor in the leftist heap. */
	...
	public ArrayList<LPoint> extractCluster() {
		...
		/* Operation contained in a try-catch block to handle potential
		 * exception thrown by leftist heap extract operation.*/
		try {
			...
			/* If the extended kd-tree contains the minimum key in the leftist
			 * heap, we simply return it. */
			if (success) {
				for (LPoint point : labeledPoints) {
					this.kdTree.delete(point.getPoint2D());
				}
				return labeledPoints;
			/* If the extended kd-tree does not contain every element in the
			 * list from above, but does contain the minimum element, create
			 * a new list of labeled points, radius, and corresponding leftist
			 * heap, and continue extracting clusters. */
			} else {
			...
			/* Theoretically, if we're doing everything right, we should ne ver
			 * reach the catch clause, since we should never be trying to
			 * extract elements from an empty leftist heap. */
			System.out.println("You're doing something wrong.");
		}
		return this.extractCluster();
	}

	/* Returns a list representation of the extended kd-tree, for debugging
	 * purposes. */
	public ArrayList<String> listKdTree() {
		return this.kdTree.list();
	}

	/* Returns a list representation of the leftist heap, for debugging
	 * purposes. */
