...
public class ExpandingStack {
	...
	/* Construct new stack by creating an empty array of given size, and setting the "top" variable to -1. "top" is used to determine whether we can continue pushing elements onto the stack, or if the stack is full and we need to resize. */
	...
	/* Push a new element onto the top of the stack. If the stack overflows, we resize by allocating a new array with twice as much space as our current one and copying existing elements over. Amortized analysis shows this runs in O(n). */
